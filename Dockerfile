...
WORKDIR /src
COPY ["payment_service.csproj", "."]
RUN dotnet restore "./payment_service.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./payment_service.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./payment_service.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "payment_service.dll"]